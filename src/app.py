...
class StreamlitApp:
    ...
    def run(self):
        ...
        tab1, tab2, tab3, tab4 = st.tabs(
            ["Prediction", "Model Performance", "Feature Importance", "About"]
        )
    ...
    def _prediction_tab(self, model_choice, compare_models):
        ...
        with col1:
            ...
            gender = st.selectbox(
                "Gender", ["Male", "Female", "Other"], help="Patient gender"
            )
            ever_married = st.selectbox(
                "Ever Married", ["Yes", "No"], help="Marital status"
            )
            ...
            residence_type = st.selectbox(
                "Residence Type", ["Urban", "Rural"], help="Type of residence"
            )
        ...
        with col2:
            ...
            bmi = st.slider("BMI", 10.0, 50.0, 25.0, 0.1, help="Body Mass Index")
        ...
        if st.button("Predict Stroke Risk", type="primary", use_container_width=True):
            ...
            try:
                ...
                if not self.data_processor.is_fitted():
                    st.error(
                        "DataProcessor is not initialized. Please train models first by running: `python src/main.py`"
                    )
                    return
                ...
                if compare_models:
                    ...
                    if logistic_model is None or rf_model is None:
                        st.error(
                            "Both models must be trained. Please run: `python src/main.py`"
                        )
                        return
                    ...
                    if lr_prediction != rf_prediction:
                        st.warning(
                            "**Models Disagree**: Different algorithms may give different predictions. Check the Performance tab to see which model performs better."
                        )
                    else:
                        st.success(
                            "**Models Agree**: Both models predict the same outcome."
                        )
                else:
                    ...
                    if model is None:
                        st.error(
                            f"{model_choice} model not trained yet. Please train the model first."
                        )
                        return
                    ...
                    if probability < 0.3:
                        st.info(
                            "**Low Risk**: The model predicts a low probability of stroke."
                        )
                    elif probability < 0.7:
                        st.warning(
                            "**Moderate Risk**: The model predicts a moderate probability of stroke. Consider consulting a healthcare professional."
                        )
                    else:
                        st.error(
                            "**High Risk**: The model predicts a high probability of stroke. Please consult a healthcare professional immediately."
                        )
    ...
    def _performance_tab(self, model_choice, compare_models):
        ...
        if compare_models or model_choice is None:
            ...
            if len(self.model_evaluator.metrics) == 0:
                st.warning(
                    "No model metrics available. Please train and evaluate models first."
                )
                return
            ...
            if (
                "Logistic Regression" in self.model_evaluator.metrics
                and "Random Forest" in self.model_evaluator.metrics
            ):
                st.markdown("---")
                ...
                self.model_evaluator.plot_roc_curve(
                    lr_metrics["y_test"],
                    lr_metrics["y_pred_proba"],
                    "Logistic Regression",
                    ax=ax,
                )
                self.model_evaluator.plot_roc_curve(
                    rf_metrics["y_test"],
                    rf_metrics["y_pred_proba"],
                    "Random Forest",
                    ax=ax,
                )
                st.pyplot(fig)
        ...
        with viz_col1:
            ...
            self.model_evaluator.plot_confusion_matrix(
                metrics["y_test"], metrics["y_pred"], model_choice, ax=ax
            )
            st.pyplot(fig)
        ...
        with viz_col2:
            ...
            fig, ax = plt.subplots(figsize=(8, 6))
            self.model_evaluator.plot_roc_curve(
                metrics["y_test"], metrics["y_pred_proba"], model_choice, ax=ax
            )
            st.pyplot(fig)
        ...
        if model_choice == "Random Forest":
            try:
                ...
                self.model_evaluator.plot_feature_importance(
                    model, model_choice, top_n=top_n, ax=ax
                )
                st.pyplot(fig)
            except Exception as e:
            ...
            st.info(
                "Feature importance visualization is available for Random Forest models only."
            )
            st.markdown("""
